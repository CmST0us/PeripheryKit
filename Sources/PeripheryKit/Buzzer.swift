...
public class Buzzer {
    ...
    public var noteGap: Float
    
    public var fixedHalfStep = 0
    ...
    public init(chip: PWM.Chip, bpm: Int = 96, noteGap: Float = 0.1) {
        guard bpm > 0 else {
            fatalError("bpm must be positive")
        }

        self.pwm = PWM(chip: chip)
        self.pwm.open()
        self.bpm = Float(bpm)
    ...
    private func tone(_ frequency: Float?, _ duration: Float, suspend: Bool = false) {
        let period: Int
